    ...
    <div class="navbar-items d-flex justify-content-start align-items-center">
        ...
        <!-- {{ category }} -->
            <a 
                ...
                @click.prevent="goToShopPage(category)"
                    ...
                    <div class="col-lg-2" v-for="(item, i) in category.children" :key="i">
                        <div class="category-item">
                            <a 
                                ...
                                @click.prevent="goToShopPage(item)"
                            >
                            ...
                            <ul class="list-unstyled">
                                <li v-for="(childItem, j) in item.children" :key="i">
                                    ...
                                    <a 
                                        class="nav-link text-black"
                                        :href="itemLink({
                                            categories: childItem.name,
                                            linkType: childItem.linkType,
                                            routeBehavior: true,
                                            isWatcher: true,
                                        })"
                                        @click.prevent="goToShopPage(childItem)"
                                    >{{childItem.name}}</a>
                                </li>
                            </ul>
                        </div>
...
const itemLink = (obj) => {
    ...
    if (obj.linkType == 'CATEGORY' && obj?.extras?.categoriesId?.length) {
        link = `/shop?category=${obj?.extras?.categories}&categoryId=${obj?.extras?.categoriesId}}`;
    } else if (obj.linkType == 'LINK') {
        link = `/shop`;
        // link = obj.name;
    } else if (obj.linkType == 'COLLECTION' && obj?.extras?.collectionsId?.length) {
        link = `/collections?collection=${obj?.extras?.collections}&collectionId=${obj?.extras?.collectionsId}}`;
        // link = obj.name;
    }
...
const goToShopPage = (obj) => {
    ...
    if (obj.linkType == 'CATEGORY') {
        router.push({
            name: "shop",
            query: {
                category: obj?.extras?.categories,
                categoryId: obj?.extras?.categoriesId,
                // linkType: category.linkType,
                // routeBehavior: true,
                // isWatcher: true,
            }
    ...
    } else if (obj.linkType == 'COLLECTION') {
        router.push({
            name: "collections",
            query: {
                collection: obj?.extras?.collections, 
                collectionId:obj?.extras?.collectionsId
            }
        })
    }
  emit('hideNav');

    ...
    <div class="">
        ...
        <div class="container-fluid">
            <div class="mx-1 mx-lg-3 mx-md-3 my-4">
                <div
                    class="d-flex justify-content-end align-items-center mx-1 mx-lg-3 mx-md-3 my-3"
                >
                    <div
                        class="d-flex justify-content-evenly align-items-center rounded-pill border border-dark filter-btn"
                ...
                <div class="row">
                    <div
                        class="col-xl-2 col-lg-2 col-md-2 col-sm-12 col-12 pt-2"
                        v-if="showFilter"
                    >
                    ...
                    <div
                        :class="{
                            'col-xl-10 col-lg-10 col-md-10 col-sm-12 col-12 pt-2':
                                showFilter,
                            'col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 pt-2':
                                !showFilter,
                        }"
                    >
                        <div class="mx-1">
                            ...
                            <div
                                v-if="
                                    fetchPageError.length > 0 && !isFetchingPage
                                "
...
const selectedCollectionsRequestBody = ref([]);
...
const categoryBanner = ref(
    route.query.categoryBanner ? route.query.categoryBanner : ""
);
...
const metaTags = computed(() => {
    ...
    if (metaTagStore.metaTags?.length) {
        let homesMeta = metaTagStore.metaTags.filter(
            (i) => i?.fields?.page && i?.fields?.page == "shop"
        );
        if (homesMeta.length) {
...
watch(
    ...
    () => {
        if (attributesStore.attributes.length) {
            allColors.value = attributesStore.attributes.filter(
                (att) => att.displayLabel.toUpperCase() == "COLOR"
            )[0];
            allSizes.value = attributesStore.attributes.filter(
                (att) => att.displayLabel.toUpperCase() == "SIZE"
            )[0];
            allBabyAges.value = attributesStore.attributes.filter(
                (att) => att.displayLabel.toUpperCase() == "BABY AGE"
            )[0];
        }
    },
    { immediate: true }
...
watch(
    ...
    (newVal) => {
        ...
        if (newVal.length) {
            ...
            for (let i = 0; i < newVal.length; i++) {
                selectedCategoriesRequestBody.value.push(newVal[i].name.trim());
            }
        } else {
            selectedCategoriesRequestBody.value = [];
        }
...
watch(
    ...
    (newVal) => {
        ...
        console.log(newVal);
        if (newVal.length) {
            selectedCollectionsRequestBody.value = [];
            let cateValues = [];
            let field = `collectionIds`;
            for (let i = 0; i < newVal.length; i++) {
                let value = newVal[i];
                cateValues.push(value.id);
            }
            ...
            } else selectedCollectionsRequestBody.value = [];
            // for (let i = 0; i < newVal.length; i++) {
            //     selectedCollectionsRequestBody.value.push(
            //         newVal[i].name.trim()
            //     );
            // }
        } else {
...
const pageItems = computed(() => {
    ...
    if (page.value && page.value.length > 0) {
        if (categoryId.value && categoryId.value.length > 0) {
            items = page.value.filter((i) => {
                if (!i) return true;
                if (!i.categoryIds) return true;

                return i.categoryIds.includes(categoryId.value);
            });
        } else items = page.value;
    }
    return items;
});
...
const filter = async (page) => {
    ...
    if (selectedCollectionsRequestBody.value?.length) {
        params.filters.push(selectedCollectionsRequestBody.value[0]);
    }
    ...
    // if (selectedFilters.value.stocks || !selectedFilters.value.stocks) {
    //     params.nonZeroStockOnly = selectedFilters.value.stocks;
    // }
    // if (selectedColorsRequestBody.value?.length) {
    //     params.filters.push(selectedColorsRequestBody.value[0]);
    // }
    // if (selectedAttributesRequestBody.value?.length) {
    //     params.filters = params.filters.concat(selectedAttributesRequestBody.value);
    // }
    // if (selectedAttributesRequestBody.value?.length) {
    //     params.filters.push(selectedAttributesRequestBody.value[0]);
    // }
...
watch(
    ...
    () => {
        if (categoryId.value) {
            ...
            selectedCategories.value = route.query?.categoryId?.length
                ? [
                      {
...
// const fillUp = (category, depth) => {
//     category.depth = depth;
//     if (category.children.length) {
//         category.isAccordion = true;
//         if (category.children && category.children.length) {
//             let children = [];
//             for (let j = 0; j < category.children.length; j++) {
//                 children.push(fillUp(category.children[j], depth + 1));
//             }
//             category.children = children.sort(
//                 (a, b) => a.sortOrder - b.sortOrder
//             );
//         }
//     } else {
//         category.isAccordion = false;
//     }
//     return category;
// };
    ...
    () => {
        if (route.query) {
            if (route.query?.query?.length) {
                if (route.query?.query == "emptyQuery") query.value = "";
                else query.value = route.query.query;

                if (route.query?.categoryName == "all") {
                    selectedCategoriesRequestBody.value = [];
                }
                filter(1);
            }

            if (route.query?.collectionId?.length) {
                console.log("collection shop page route.query?.collectionId");
                console.log(route.query?.collectionId);
                selectedCollections.value = [{ id: route.query?.collectionId }];
                filter(1);
            }
            ...
            //         ? route.query.categoryName
            //         : "";
            // categoryBanner.value = route.query?.categoryBanner?.length
            //     ? route.query.categoryBanner
            //     : "";
            ...
            // }
            // toaster.success(`${categoryName.value}`, { duration: 1500 });
        }
    },

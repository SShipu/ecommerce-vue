...
const successCall = (id, invoiceId) => {
    let data = {
        id: invoiceId,
    }
    ordersPlaceClient
        .confirmPayment(data, orderType.value)
        .then((res) => {
...
watch(route, ()=> {
    ...
    if (route.query && route.query.invoiceId && route.query.invoiceId.length) {
        invoiceId.value = route.query.invoiceId;
        orderType.value = route.query.type;
        id.value = route.query.id;
        cartStore.removeAll();
        successCall(route.query.id, route.query.invoiceId);
    }
}, {immediate: true, deep: true});

onMounted(() => {
    let checkoutRes = localStorage.getItem('checkout');
    if (checkoutRes) {
        let ckLs = JSON.parse(checkoutRes);
        total.value = ckLs.total;
        referenceId.value = ckLs.referenceId;
    }
})
